...
function Point(x, y, z, size, color) {
    ...
    this.draw = function (bubbleShape, dx, dy) {
        ...
        if (bubbleShape == "square") {
            ctx2.beginPath();
            ctx2.fillRect(this.curPos.x + dx, this.curPos.y + dy, this.radius * 1.5, this.radius * 1.5);
        } else {
            ctx2.beginPath();
            ctx2.arc(this.curPos.x + dx, this.curPos.y + dy, this.radius, 0, Math.PI * 2, true);
            ctx2.fill();
        }
    };
}
